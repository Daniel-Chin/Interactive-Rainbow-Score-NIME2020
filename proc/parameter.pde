...
static class Parameter {  // according to modes.md
  ...
  static int BLUETOOTH_MIDI_BUF_MS = 200;
  static int PROC2ARDU_LATENCY = 100;  // 38,67
  static int capacitive_threshold = 3;
...
class SceneParameter extends Layer {
  ...
  class Row extends Layer {
    ...
    Row(int index, int _max, String _text, int init) {
      ...
      this.add(new RowSlider(_max, init, this));
  ...
  class CapacitiveRow extends Row {
    CapacitiveRow(int index) {
      super(index, 8, "Capacitive Threshold", Parameter.capacitive_threshold);
    }
    void onSliderChange(int value) {
      Parameter.capacitive_threshold = value;
      arduino.setCapacitiveThreshold(value);
    }
  }
