...
class RainbowSheet extends Layer{
  ...
  class RainbowBand extends Pressable {
    ...
    void draw() {
      ...
      if (! show_discrete && Parameter.interactive_visual) {
        ...
        for (Session.CYP cyp : session.cyps) {
          cyp.draw(this);
        }
        // draw arrow
        // if (
        //   hapticManager.haptic != null 
        //   && ! hapticManager.haptic.progressOnTimeOrSequence()
        //   && session.lastCYP != null 
      ...
      // draw stem beam
      if (!Parameter.back_flashlight || !hapticManager.is_playing) {
      ...
      popMatrix();
      if ((start() <= score.time || y1 == 0) && score.time < end()) {
        pushMatrix();
    ...
    void drawFluteColor() {
      int note_index = score.play_head;
      if (note_index == -1) {
        note_index = 0;
      }
      int pitch;
    ...
    void drawCursor() {
      strokeWeight(.01); stroke(director.themeFore);
      ...
      if (! show_discrete && cursorTime() > note_on) {
        ...
        if (! is_rest) {
          ...
          rect( // Voldemort
            x, 
            y - .8 * METRIC_UNIT,
            band.time2Ratio(min(note_off, score.time)) - x,
            METRIC_UNIT * 1.6
          );
        }
        return;
  ...
  void jumpTo(int desired) {
    if (((SceneMain) parent).is_review_playing) {
      score.time = desired;
      playback.reload();
    } else if (hapticManager.is_playing) {
      if (hapticManager.haptic.progressOnTimeOrSequence()) {
        hapticManager.haptic.start_time -= (desired - score.time) / Parameter.tempoMuliplier();
        score.play_head = -1;
        if (USE_BGM && bgm != null) {
          bgm.jump(desired / 1000f + score.prelude / 1000f);
        }
      } else {
        score.play_head = score.time2Note_i(desired, true, false);
      }
      hapticManager.analysis.add("playhead_jump", 0, 0, 'j');
    } else {
      score.time = desired;
    }
  }
  ...
  class BandSelect extends ScrollSelect {
    ...
    void draw() {
      value = max(0, min(range, score.time / time_per_band));
      super.draw();
    }
